			...
			if(fr !== null) {
				...
				if (v2 !== $t.p.savedRow[fr].v){
					...
					if(cv[0] === true) {
					    ...
					    try {
					        // WARNING 02/11 - Modificado por Gabriel para editar el titulo de validacion inline.
					        var incValue = "<strong id='errValue'>" + v + "</strong>";
					        var top = window.innerHeight / 2 - 50;
					        var left = (window.innerWidth / 2) - 150;
					        var valErrorOpt = { left: left, top: top }
					        window.setTimeout(function () { $.jgrid.info_dialog($.jgrid.errors.valerrcap, incValue + " " + cv[1], $.jgrid.edit.bClose, valErrorOpt); }, 100);
							$($t).jqGrid("restoreCell",iRow,iCol);
						} catch (e) {}
					}
				} else {
